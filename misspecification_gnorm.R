...
sim.prs <- mclapply(prs, function(pr) {
  ...
  for (s2c in s2cs) {
    ...
    for (i in 1:sims) {
      ...
      a<- rep(0, n)
      b<- rep(0, p)
      C<-matrix(s2c*rgnorm(n*p, mu = 0, beta = pr, alpha = sqrt(gamma(1/pr)/gamma(3/pr))),
                nrow = n, ncol = p)
...
save.image(paste("misspecification_gnorm_", k, ".RData", sep = ""))
